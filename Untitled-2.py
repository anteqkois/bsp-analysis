...
def calculate_trailing_exit(entry_row, rwos, trailing_stop_loss, stop_loss, fixed_profit,
    ...
    for row in rwos:
        ts, h, l , c = row  # Assuming prices is a 2D array with columns [ts, h, l, c]
        ...
        if use_stop_loss and l <= entry_c * (1 - stop_loss / 100):
            return {'price': entry_c * (1 - stop_loss / 100), 'exit_reason': 'Stop Loss'}
...
def calculate_fixed_exit(entry_row, rows, fixed_profit, fixed_loss, use_stop_loss=False, use_fixed_profit=False,
    ...
    for row in rows:
        ...
        if use_stop_loss and l <= entry_c * (1 - fixed_loss / 100):
            return {'price': entry_c * (1 - fixed_loss / 100), 'exit_reason': 'Stop Loss', 'ts': ts}
...
def trade_simulation(
    ...
    for _, point in points_below_min.iterrows():
        ...
        trail_exit = calculate_trailing_exit(point, data_after_entry, Y_trail_profit, Y_stop_loss, Y_trail_fixed_profit, use_trailing_stop_loss, use_stop_loss, use_fixed_profit,  use_max_bsp_exit, points_above_max_preprocessed)
        ...
        fixed_exit = calculate_fixed_exit(point, data_after_entry, Y_fixed_profit, Y_fixed_loss, use_stop_loss, use_fixed_profit, use_max_bsp_exit, points_above_max_preprocessed)
